...
void UAT_PlayMontageAndWaitForEvent::Activate()
{
	...
	UPTAbilitySystemComponent* PTAbilitySystemComponent = GetTargetASC();

	if (PTAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = PTAbilitySystemComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UAT_PlayMontageAndWaitForEvent::OnGameplayEvent));

			if (PTAbilitySystemComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
void UAT_PlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (PTAbilitySystemComponent)
	{
		PTAbilitySystemComponent->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
...
UPTAbilitySystemComponent* UAT_PlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<UPTAbilitySystemComponent>(AbilitySystemComponent);
